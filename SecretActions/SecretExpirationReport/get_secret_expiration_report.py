...
def create_report(expiration_date: str, records: list[dict]) -> str:
    ...
    env = Environment(loader=FileSystemLoader(
        "SecretActions/SecretExpirationReport/Files/"))
    template = env.get_template("ReportTemplate.html")
    logging.info("Successfully loaded template.")
    content = template.render(
        records=records, expiration_date=expiration_date, current_date=current_date)
...
def get_soon_to_expire_secrets(gsm_client: GoogleSecreteManagerModule, expiration_date: str,
                               project_id: str = DEV_PROJECT_ID) -> list[dict]:
    """
    Args:
    ...
    q = f"(labels.{ExpirationData.CREDS_EXPIRATION_LABEL_NAME}<{expiration_date} OR " \
...
def options_handler(args=None):
    ...
    parser.add_argument('-e', '--expiration',
                        help='The expiration time of the secret in the following format: yyyy-mm-dd.')
    parser.add_argument('-sa', '--service_account',
                        help=("Path to gcloud service account, for the flow usage. "
