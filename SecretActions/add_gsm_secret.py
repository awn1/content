...
DEFAULT_PR_COMMENT = f'The "{SYNC_GSM_LABEL}" label has been added to your PR.' \
...
def validate_secret(secret: str, options: argparse.Namespace, project_id, attr_validation: tuple,
    ...
    missing_attrs = [
        attr for attr in attr_validation if attr not in json_object]
    if missing_attrs:
        raise Exception(
    ...
    if cred_expiration := json_object.get(ExpirationData.CREDS_EXPIRATION_LABEL_NAME):
        res = GoogleSecreteManagerModule.GoogleSecretTools.calculate_expiration_date(
            cred_expiration)
        if not res:
    ...
    if options.pr_number:
        if project_id != DEV_PROJECT_ID:
            raise Exception(
                f'Pull Request number is not supported for a prod secret, you provided {project_id=}.')
        try:
            if github_client:
    ...
    for label in labels_to_add:
        if label_value := options_labels.get(label):
            if label in (ExpirationData.CREDS_EXPIRATION_LABEL_NAME, ExpirationData.LICENSE_EXPIRATION_LABEL_NAME):
                label_value = GoogleSecreteManagerModule.GoogleSecretTools.calculate_expiration_date(
                    label_value)
...
def upsert_secret(gsm_object: GoogleSecreteManagerModule, options: argparse.Namespace, gsm_project_id: str,
    ...
    instance = f'__{secret_json.get("instance_name")}' if secret_json.get(
        'instance_name') else ''
    full_secret_name = gsm_object.convert_to_gsm_format(
        f'{secret_json.get("name", )}{instance}', secret_name=True)
    ...
    gsm_object.add_secret_version(
        gsm_project_id, full_secret_name, json5.dumps(secret_json, quote_keys=True))
...
def add_secret_to_dev(options: argparse.Namespace, project_id: str):
    ...
    print(YELLOW_BOLD_PRINT + f"The secret was successfully added to GSM, and the '{SYNC_GSM_LABEL}' label has been \
            \nadded to your PR. If you decide to not merge the new secret you added, make sure to remove \
            \nthis label before closing the PR." + YELLOW_BOLD_PRINT)
...
def options_handler(args=None):
    ...
    parser.add_argument(
        '-i', '--input', help='The secret json file path with the secret value we want to add.')
    parser.add_argument('-pr', '--pr_number',
                        help='The pr number to add the secret for, required when adding a dev secret.',
                        required=False)
