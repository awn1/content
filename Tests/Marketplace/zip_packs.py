...
def option_handler():
    ...
    parser.add_argument("-a", "--artifacts_path", help="Path of the CircleCI artifacts to save the zip file in", required=False)
        ...
        for name, path in zip_pack.items():
            ...
            with ZipFile(path, mode="r") as pack_zip:
                ...
                if "TestPlaybooks" in dir_names:
                    ...
                    pack_zip.extractall(
                        path=pack_path, members=(member for member in zip_contents if "TestPlaybooks" not in member)
                    )
