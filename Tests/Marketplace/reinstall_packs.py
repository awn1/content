...
def reinstall_packs(options: argparse.Namespace, cloud_machine: str) -> bool:
    ...
    """
    success = True
    api_key, _, base_url, xdr_auth_id, _ = CloudBuild.get_cloud_configuration(cloud_machine, options.cloud_servers_path)
    ...
    for pack in packs_to_reinstall:
        ...
        _, successful_install = search_and_install_packs_and_their_dependencies(
            pack_ids=[pack], client=client, hostname=host, install_packs_in_batches=True, production_bucket=False
        )
...
def main():
    ...
    with ThreadPoolExecutor(max_workers=len(cloud_machines), thread_name_prefix="clean-machine") as executor:
        futures = [executor.submit(reinstall_packs, options, cloud_machine) for cloud_machine in cloud_machines]
