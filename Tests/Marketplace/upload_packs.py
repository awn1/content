...
def get_packs_ids_to_upload_and_update(packs_to_upload: str) -> tuple[set, set]:
    ...
    if packs_to_upload and isinstance(packs_to_upload, str):
        try:
            ...
            logging.debug(
                f"Collected {len(packs_metadata_update)} content packs to update metadata: " f"{list(packs_metadata_update)}"
            )
...
def download_and_extract_index(
    ...
    if os.path.exists(download_index_path):
        ...
        logging.success(f"Finished downloading and extracting {GCPConfig.INDEX_NAME} file to " f"{extract_destination_path}")
    ...
    try:
        ...
        if os.path.exists(index_pack_path):
            for d in os.scandir(index_pack_path):
                if (
                    (metadata_version := re.findall(METADATA_FILE_REGEX_GET_VERSION, d.name))
                    and pack_versions_to_keep
                    and metadata_version[0] not in pack_versions_to_keep
                ):
...
def clean_non_existing_packs(
    ...
    invalid_packs_names = {
        ...
        if entry.name not in valid_pack_names and entry.is_dir()
    }
...
def upload_index_to_storage(
    ...
    index_zip_path = shutil.make_archive(
        base_name=index_folder_path, format="zip", root_dir=extract_destination_path, base_dir=index_zip_name
    )
    try:
...
def build_summary_table_md(packs_input_list: list, include_pack_status: bool = False) -> str:
    ...
    """
    table_fields = (
        ["Index", "Pack ID", "Pack Display Name", "Latest Version", "Status"]
        if include_pack_status
    ...
    for index, pack in enumerate(packs_input_list):
        ...
        row = (
            [index, pack.name, pack.display_name, pack.current_version, pack_status_message]
            if include_pack_status
...
def check_if_index_is_updated(
    index_folder_path: str, content_repo: Any, current_commit_hash: str, previous_commit_hash: str, storage_bucket: Any
):
    ...
    try:
        ...
        for changed_file in current_commit.diff(index_commit):
            if changed_file.a_path.startswith(PACKS_FOLDER):
                logging.debug(f"Found changed packs between index commit {index_commit.hexsha} and {current_commit.hexsha}")
                break
        else:
...
def print_packs_summary(successful_packs: list, skipped_packs: list, failed_packs: list, fail_build: bool = True):
...
----------------------------------------------------------------------------------------------------------"""
    ...
    if branch_name and branch_name.startswith("pull/"):
        ...
        bucket_path = (
            f"https://console.cloud.google.com/storage/browser/" f"marketplace-ci-build/content/builds/{branch_name}/{build_num}"
        )
...
def upload_packs_with_dependencies_zip(storage_bucket, storage_base_path, signature_key, packs_dict):
    """
    ...
    for pack_name, pack in packs_dict.items():
        try:
            if (pack.status not in [*SKIPPED_STATUS_CODES, PackStatus.SUCCESS.name]) or pack.hidden:
                ...
                continue
            pack_and_its_dependencies = [packs_dict.get(dep_name) for dep_name in pack.all_levels_dependencies] + [pack]
            pack_or_dependency_was_uploaded = any(
                dep_pack.status == PackStatus.SUCCESS.name for dep_pack in pack_and_its_dependencies
            )
            if pack_or_dependency_was_uploaded:
                ...
                task_status = pack.upload_to_storage(
                    zip_with_deps_path, storage_bucket, storage_base_path, with_dependencies_path=upload_path
                )
                logging.debug(f"{pack_name} with dependencies was{'' if task_status else ' not'} " f"uploaded successfully")
                if not task_status:
...
def delete_from_index_packs_not_in_marketplace(index_folder_path: str, current_marketplace_packs: list[Pack]):
    """
...
def option_handler():
    ...
    parser.add_argument("-pn", "--pack_names", help=("Target packs to upload to gcs."), required=True)
    parser.add_argument(
        "-p",
        "--upload_specific_pack",
        type=str2bool,
        help=("Indication if the -p flag is used and only specific packs are uploded"),
        default=False,
    )
    parser.add_argument(
        "-n", "--ci_build_number", help="CircleCi build number (will be used as hash revision at index file)", required=False
    )
    parser.add_argument(
        "-o",
        "--override_all_packs",
    ...
    parser.add_argument("-k", "--key_string", help="Base64 encoded signature key used for signing packs.", required=False)
    parser.add_argument("-sb", "--storage_base_path", help="Storage base path of the directory to upload to.", required=False)
    parser.add_argument(
        "-rt", "--remove_test_deps", type=str2bool, help="Should remove test playbooks from content packs or not.", default=True
    )
    ...
    parser.add_argument(
        "-dz", "--create_dependencies_zip", type=str2bool, help="Upload packs with dependencies zip", required=False
    )
...
def main():
    ...
    markdown_relative_images_data = (
        Path(option.artifacts_folder_server_type) / BucketUploadFlow.MARKDOWN_RELATIVE_IMAGES_FILE_NAME
    )
    ...
    current_commit_hash, previous_commit_hash = get_recent_commits_data(
        content_repo, index_folder_path, is_bucket_upload_flow, ci_branch
    )
    ...
    all_packs_objects_list = [
        Pack(
            pack_id,
            os.path.join(extract_destination_path, pack_id),
    ...
    if is_regular_upload_flow:
        check_if_index_is_updated(index_folder_path, content_repo, current_commit_hash, previous_commit_hash, storage_bucket)
    ...
    for pack in packs_objects_list:
        ...
        if not pack.enhance_pack_attributes(
            index_folder_path, packs_dependencies_mapping, marketplace, statistics_handler, remove_test_deps
        ):
        ...
        task_status, not_updated_build, pack_versions_to_keep = pack.prepare_release_notes(
            index_folder_path, build_number, diff_files_list, marketplace, id_set, is_override=override_all_packs
        )
        ...
        if pack.is_modified:
            ...
            if not download_and_extract_pack(
                pack.name, pack.current_version, storage_bucket, extract_destination_path, storage_base_path
            ):
    ...
    if is_create_dependencies_zip and marketplace == XSOAR_MP:
        ...
        upload_packs_with_dependencies_zip(storage_bucket, storage_base_path, signature_key, all_packs_dict)
