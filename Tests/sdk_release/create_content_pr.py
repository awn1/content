...
SLACK_RELEASE_MESSAGE = (
    "demisto-sdk `{sdk_version}` has been released :party-github:\n"
...
def get_pr_from_branch(repository, branch, access_token):
    ...
    res = requests.get(url, headers={"Authorization": f"Bearer {access_token}"}, params=params, verify=False)
...
def main():
    ...
    slack_message = SLACK_MERGE_PRS_MESSAGE.format(content_pr=content_pr.get("html_url"), sdk_pr=sdk_pr.get("html_url"))
