...
def options_handler():
    parser = argparse.ArgumentParser(
        description="Triggers update-demisto-sdk-version workflow"
    )

    parser.add_argument(
        "-t", "--github_token", help="Github access token", required=True
    )
    parser.add_argument(
        "-v", "--release_version", help="The release version", required=True
    )
    parser.add_argument(
        "-r", "--reviewer", help="The reviewer of the pull request", required=True
    )
    parser.add_argument(
        "-b",
        "--sdk_branch_name",
...
def main():
    ...
    if response.status_code != requests.codes.ok:
        errors.append(
            f"Failed to retrieve the user {reviewer} from github,\nerror: {response.text}"
        )
    ...
    if response.status_code != requests.codes.ok:
        errors.append(
            f"Failed to retrieve the branch {sdk_branch_name} from demisto-sdk repo,\nerror: {response.text}"
        )
