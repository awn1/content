...
poetry run python3 -u ./Tests/scripts/lock_cloud_machines.py --service_account $GCS_ARTIFACTS_KEY --gcs_locks_path $GCS_LOCKS_PATH  --ci_job_id $CI_JOB_ID --ci_pipeline_id $CI_PIPELINE_ID  --test_machines $TEST_MACHINES_LIST_STRING  --gitlab_status_token $GITLAB_API_TOKEN_CONTENT  --lock_machine_name "${LOCK_MACHINE_NAME}" --number_machines_to_lock ${CLOUD_MACHINES_COUNT} --response_machine CloudEnvVariables