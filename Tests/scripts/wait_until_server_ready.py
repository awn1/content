...
def docker_login(ip: str) -> None:
    ...
    try:
        ...
        logging.exception(
            f"Could not login to {container_engine_type} on server {ip}:\n{e.stderr=}\n{e.output=}\n"
            f"{e.returncode=}\n{e.cmd=}."
        )
...
def main():
    ...
    instance_ips_to_poll = [
        ami_instance_ip for ami_instance_name, ami_instance_ip in instance_ips if ami_instance_name == args.instance_role
    ]
