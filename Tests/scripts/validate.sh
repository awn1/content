...
if [[ $CI_COMMIT_BRANCH = master ]] || [[ "${IS_NIGHTLY}" == "true" ]] || [[ "${BUCKET_UPLOAD}" == "true" ]] || [[ "${DEMISTO_SDK_NIGHTLY}" == "true" ]] || [[ "${SHOULD_VALIDATE_ALL}" == "true" ]]; then
  if [[ -n "${PACKS_TO_UPLOAD}" ]]; then
    ...
    PACKS_TO_UPLOAD_SPACED="Packs/${PACKS_TO_UPLOAD//,/,Packs/}"
    python3 -m demisto_sdk validate -i "${PACKS_TO_UPLOAD_SPACED}" --post-commit --config-path validation_config.toml
  else
