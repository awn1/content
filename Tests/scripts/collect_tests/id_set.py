...
from Tests.scripts.collect_tests.utils import DictBased, DictFileBased, PackManager, find_pack_folder, to_tuple
...
class IdSetItem(DictBased):
    ...
    def __init__(self, id_: str | None, dict_: dict):
        ...
        self.deprecated: bool | None = self.get("deprecated", warn_if_missing=False) or self.get("hidden", warn_if_missing=False)
    ...
    def _calculate_pack_path(path: Path | None) -> Path | None:
        ...
        try:
            ...
            if path.name in SKIPPED_CONTENT_ITEMS__NOT_UNDER_PACK:
                logger.info(f"{path=} is not under a pack, " "but is part of SKIPPED_CONTENT_ITEMS__NOT_UNDER_PACK, skipping")
                return None
            else:
                raise
    ...
    def implementing_integrations(self) -> tuple[str, ...]:
        ...
        for _command, integrations in self.get("command_to_integration", {}, warn_if_missing=False).items():
            result.update((integrations,) if isinstance(integrations, str) else integrations)
                    ...
                    for value in values:  # may have multiple values for different from/to versions
                        ...
                        if existing := result.get(id_):
                            ...
                            if item.to_version <= existing.to_version and item.from_version <= existing.from_version:
                                logger.debug(
                                    f"skipping duplicate of {item.name} as its version range "
...
class Graph:
    def __init__(self, marketplace: MarketplaceVersions) -> None:
        ...
        with Neo4jContentGraphInterface() as content_graph_interface:
            integrations = content_graph_interface.search(marketplace=marketplace, content_type=ContentType.INTEGRATION)
            scripts = content_graph_interface.search(
                marketplace=marketplace, content_type=ContentType.SCRIPT, all_level_imports=True
            )
            test_playbooks = content_graph_interface.search(marketplace=marketplace, content_type=ContentType.TEST_PLAYBOOK)
            ...
            self.path_to_parsing_rule = {parsing_rule.path: IdSetItem.from_model(parsing_rule) for parsing_rule in parsing_rules}
            self.implemented_playbooks_to_tests = {
                playbook.object_id: [IdSetItem.from_model(test) for test in playbook.tested_by] for playbook in playbooks
            }
            self.implemented_scripts_to_tests = {
                script.object_id: [IdSetItem.from_model(test) for test in script.tested_by] for script in scripts
            }
