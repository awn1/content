...
def create_jira_issue_for_test_modeling_rule(
    ...
    if jira_issue is not None:
        if test_suite.failures == 0 and test_suite.errors == 0 and (resolution := jira_issue.get_field("resolution")) is not None:
            logging.info(
                f"Skipping updating Jira issue {jira_issue.key} as it has no failures or errors "
    ...
    with NamedTemporaryFile() as attachment_file_name:
        ...
        jira_server.add_attachment(
            issue=jira_issue.key, attachment=attachment_file_name.name, filename=junit_file_name_with_suffix
        )

    back_link_to = f" with back link to {link_to_issue.key}" if link_to_issue else ""
    logging.info(
        f"{'Updated' if use_existing_issue else 'Created'} Jira issue: {jira_issue.key} {back_link_to}"
