...
ALLOWED_USERS = ["content-bot", "svc -xsoar-gitlab-mirror", "svc-xsoar-gitlab-mirror"]
...
def get_group_members(permission_group: str, github_token: str, verify_ssl: bool) -> list:
    ...
    pr_files = [f.get("login") for f in permissions_response]
    return pr_files
...
def main():
    ...
    gitlab_user = get_slack_user_name(args.user, None, args.name_mapping_path) or args.user
    if gitlab_user in gitlab_allowed_users:
        print(f"User '{gitlab_user}' is allowed to trigger")
    else:
        print(f"User '{gitlab_user}' is not allowed to trigger the flow as it is not part of {gitlab_allowed_users}")
        sys.exit(1)
