...
def options_handler() -> argparse.Namespace:
    ...
    parser.add_argument("--creds", help="GCP creds", required=True, type=str)
    parser.add_argument("--zone", help="GCP zone", required=True, type=str)
...
def get_all_pipelines_for_all_statuses(
    project: Project, branch_name: str, triggering_sources: list[str]
...
def delete_all_instances_for_pipeline(
    instances_service: InstancesService, all_instances: dict[str, compute_v1.Instance], pipeline_id: int
):
...
def get_pipeline_variables(pipeline: ProjectPipeline):
    return {variable.key: variable.value for variable in pipeline.variables.list(all=True)}


def cancel_pipelines_for_branch_name(
    ...
    pipelines = get_all_pipelines_for_all_statuses(project, branch_name, triggering_sources)
    if not pipelines:
        logging.info(f"No pipelines found for branch:{branch_name}, triggering sources:{','.join(triggering_sources)}")
        return True

    logging.info(f"Found {len(pipelines)} pipelines for branch:{branch_name}, triggering sources:{','.join(triggering_sources)}")
    success = True
        ...
        if not pipeline_id or pipeline.id < pipeline_id:
            try:
                ...
                if pipeline.status in GITLAB_STATUSES_TO_CANCEL:
                    ...
                    logging.warning(
                        f"Pipeline id:{pipeline.id} for branch:{branch_name} was not canceled because it's "
            ...
            if cancel_children:
                ...
                success &= cancel_pipelines_for_branch_name(
                    gitlab_client, project, test_upload, ["trigger"], instances_service, all_instances, False
                )
...
def main():
    try:
        ...
        gitlab_client = Gitlab(options.url, private_token=options.ci_token, ssl_verify=GITLAB_SSL_VERIFY)
        ...
        if not cancel_pipelines_for_branch_name(
            gitlab_client,
            project,
            options.current_branch,
            CONTENT_BUILD_TRIGGERING_SOURCES,
            instances_service,
            all_instances,
            True,
            pipeline_id,
