...
def test_calculate_average_execution_time(mocker):
    ...
    avg_times = calculate_average_execution_time(ARTIFACTS_BUCKET, "server_type", test_playbooks, mock_storage_client,
                                                 MAX_REPORT_FILES)
...
def test_build_pack_information():
    ...
    for pack_name, expected_pack_info in expected_packs.items():
        ...
        assert actual_pack_info.test_playbooks_to_run == expected_pack_info.test_playbooks_to_run, \
            f"test_playbooks_to_run for '{pack_name}' does not match"
        assert actual_pack_info.dependencies == expected_pack_info.dependencies, \
            f"dependencies for '{pack_name}' does not match"
        assert actual_pack_info.total_expected_execution_time == expected_pack_info.total_expected_execution_time, \
...
def test_machine_assignment():
    ...
    marketplace = 'xsoar'
    expected_machine_split_1 = {'packs_to_install': ['pack2', 'DeveloperTools', 'pack3', 'Base'],
                                'playbooks_to_run': ['playbook3']}
    expected_machine_split_2 = {'packs_to_install': ['pack1', 'pack2', 'DeveloperTools', 'Base'],
    ...
    for machine in assignments.values():
        assert set(machine['packs_to_install']) in [set(expected_machine_split_1['packs_to_install']),
                                                    set(expected_machine_split_2['packs_to_install'])]
        assert set(machine['playbooks_to_run']) in [set(expected_machine_split_1['playbooks_to_run']),
