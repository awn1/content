...
from Tests.scripts.split_packs_to_machines import (
    ARTIFACTS_BUCKET,
    MAX_REPORT_FILES,
    TEST_PLAYBOOKS_DEFAULT_EXECUTION_TIME,
    PackInfo,
    Tests,
...
def test_calculate_average_execution_time(mocker):
    ...
    <testsuites>
        <testsuite name="playbook1" tests="1" errors="0" failures="0" skipped="1" time="0.0">
            <properties>
                <property name="playbook_id" value="playbook1"/>
            </properties>
        </testsuite>
        <testsuite name="playbook2" tests="1" errors="0" failures="0" skipped="1" time="0.001">
            <properties>
                <property name="playbook_id" value="playbook2"/>
            </properties>
        </testsuite>
    </testsuites>
    """
    ...
    <testsuites>
        ...
        <testsuite name="playbook3" tests="1" errors="0" failures="0" skipped="0" time="433.691">
            <properties>
                <property name="playbook_id" value="playbook3"/>
            </properties>
        </testsuite>
    </testsuites>
    """
    ...
    mocker.patch("Tests.scripts.split_packs_to_machines.load_json", return_value=["playbook1", "playbook2", "playbook3"])

    test = calculate_average_execution_time(
        ARTIFACTS_BUCKET,
        "server_type",
        "playbook_id",
        "xsoar",
        Path("fake_file_being_mocked.json"),
        TEST_PLAYBOOKS,
        mock_storage_client,
        MAX_REPORT_FILES,
        TEST_PLAYBOOKS_DEFAULT_EXECUTION_TIME,
    ...
    assert test.test_times["playbook1"] == 429.734
    assert test.test_times["playbook2"] == 504.126
    assert test.test_times["playbook3"] == 491.5815
...
def test_build_pack_information():
    ...
    expected_packs["pack1"].tests[TEST_PLAYBOOKS] = {"playbook1", "playbook2"}
    expected_packs["pack1"].dependencies = {"pack2", "pack3", "pack4"}
    expected_packs["pack1"].total_expected_execution_time = 300.0
    expected_packs["pack2"].tests[TEST_PLAYBOOKS] = {"playbook3"}
    expected_packs["pack2"].dependencies = set()
    expected_packs["pack2"].total_expected_execution_time = 150.0

    tests = [Tests(TEST_PLAYBOOKS, playbooks, playbook_times)]
    packs_objects_to_install = build_pack_information(tests)
    for pack_name, expected_pack_info in expected_packs.items():
        ...
        assert (
            actual_pack_info.tests[TEST_PLAYBOOKS] == expected_pack_info.tests[TEST_PLAYBOOKS]
...
def test_machine_assignment():
    ...
    """
    pack1 = PackInfo("pack1")
    pack1.tests[TEST_PLAYBOOKS] = {"playbook1", "playbook2"}
    pack1.dependencies = {"pack2"}
    pack1.total_expected_execution_time = 300.0

    pack2 = PackInfo("pack2")
    pack2.tests[TEST_PLAYBOOKS] = {"playbook3"}
    ...
    assignments = machine_assignment(packs_objects_to_install, machines, packs_to_install_from_file, MarketplaceVersions.XSOAR)

    for machine in assignments.values():
        assert machine.packs_to_install in [
            set(expected_machine_split_1["packs_to_install"]),
            set(expected_machine_split_2["packs_to_install"]),
        ]
        assert machine.tests[TEST_PLAYBOOKS] in [
