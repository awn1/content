...
def machine_assignment(
    ...
    """
    machine_assignments: dict[str, dict[str, set[str]]] = {
        machine: {"packs_to_install": set(), "playbooks_to_run": set()} for machine in machines
    }
    ...
    final_assignments: dict[str, dict[str, list[str]]] = {
        machine: {
            "packs_to_install": list(machine_dict["packs_to_install"] | set(ALWAYS_INSTALLED_PACKS_MAPPING[marketplace])),
...
def calculate_average_execution_time(
    bucket_name: str, server_type: str, test_playbooks: list, storage_client: storage.Client, max_files: int
) -> dict[str, float]:
    """
    ...
    average_times = {
        playbook: sum(times) / len(times) if times else DEFAULT_PLAYBOOK_EXECUTION_TIME
...
def options_handler() -> argparse.Namespace:
    ...
    parser.add_argument(
        "--playbooks_to_packs",
        help="Path to the tpb_dependencies_packs.json file that contains " "connection between tpb to related packs to install",
        required=True,
    )
...
def main():
    ...
    playbook_times = calculate_average_execution_time(
    ...
    machine_assignments = machine_assignment(packs_objects_to_install, set(machine_list), packs_to_install, marketplace)
