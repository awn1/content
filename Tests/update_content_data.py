...
def options_handler():
    ...
    parser.add_argument(
        "-s", "--server", help="The server to connect to (leaving out the protocol e.g." " without 'https://')", required=True
    )
...
def update_content(content_zip_path, server=None, username=None, password=None, client=None):
    ...
    try:
        ...
        logging.info(f'Making "POST" request to server - "{server}" to upload the content zip file "{content_zip_path}"')
        response_data, status_code, _ = client.api_client.call_api(
            resource_path="/content/upload", method="POST", header_params=header_params, files=files
        )
