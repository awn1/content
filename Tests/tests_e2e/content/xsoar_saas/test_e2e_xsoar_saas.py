...
def test_edl_returns_indicators(request: SubRequest, xsoar_saas_client: XsoarSaasClient):
    ...
    """
    feed_indicators = [
        FeedIndicator(value=value, type=_type, score=score)
        for value, _type, score in [("1.1.1.1", "IP", 0), ("2.2.2.2", "IP", 0), ("3.3.3.3", "IP", 0)]
    ]

    with save_indicators(xsoar_saas_client, indicators=feed_indicators):
        integration_params = get_integration_params(request.config.option, instance_name="edl_e2e_instance")
            ...
            with save_integration_instance(
                ...
                try:
                    ...
                    assert edl_response.status_code == 200, (
                        f"status code is {edl_response.status_code} " f"from EDL in URL {edl_response.request.url}"
                    )
...
def test_taxii2_server_returns_indicators(
    ...
    with save_indicators(xsoar_saas_client, indicators=feed_indicators):
        integration_params = get_integration_params(request.config.option, instance_name="taxii2server_e2e_instance")
        ...
        try:
            for port in ("8000", "8001", "8002", "8003", "8004"):
                ...
                with save_integration_instance(
                    ...
                    assert collections, (
                        f"Could not get collections from url={response.request.url}, "
                    ...
                    assert indicators, (
                        f"could not get indicators from url={response.request.url} with available "
...
def test_slack_ask(request: SubRequest, xsoar_saas_client: XsoarSaasClient):
    ...
    """
    integration_params = get_integration_params(request.config.option, instance_name="slack_e2e_instance")
...
def test_qradar_mirroring(request: SubRequest, xsoar_saas_client: XsoarSaasClient):
    ...
    """
    integration_params = get_integration_params(request.config.option, instance_name="qradar_e2e_instance")
    ...
    with save_integration_instance(
        ...
        with get_fetched_incident(
            ...
            war_room_entries, context = xsoar_saas_client.run_cli_command(
                close_offense_command, investigation_id=investigation_id
            )
            assert context.get("QRadar", {}).get("Offense", {}).get("Status") == "CLOSED", (
                f"Error validating context when running "
