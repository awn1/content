...
python3 ./Tests/test_content.py -k "$DEMISTO_API_KEY" -c "$CONF_PATH" -e "$SECRET_CONF_PATH" -n "${IS_NIGHTLY}" -t "$SLACK_TOKEN" -a "$CIRCLECI_TOKEN" -b "$CI_JOB_ID" -g "$CI_COMMIT_BRANCH"
