...
def test_integration_instance(client, module_instance, logging_module=logging):
    ...
    logging_module.info(f'Running "test-module" for instance "{instance_name}" of integration "{integration_of_instance}".')
    for i in range(connection_retries):
        try:
            response_data, response_code, _ = demisto_client.generic_request_func(
                self=client, method="POST", path="/settings/integration/test", body=module_instance, _request_timeout=240
            )
            break
        except ApiException:
...
def __delete_integration_instance_if_determined_by_name(client, instance_name, logging_manager):
    ...
    try:
        int_resp = demisto_client.generic_request_func(
            self=client, method="POST", path="/settings/integration/search", body={"size": 1000}
        )
...
def __create_integration_instance(
    server,
    username,
    password,
    integration_name,
    ...
    try:
        res = demisto_client.generic_request_func(
            self=integration_conf_client, method="PUT", path="/settings/integration", body=module_instance
        )
    except ApiException:
    ...
    if validate_test:
        ...
        logging_manager.debug(f"Skipping test validation for integration: {integration_name} (it has test_validate set to false)")
        test_succeed = True
...
def __disable_integrations_instances(client, module_instances, logging_module=logging):
    for configured_instance in module_instances:
        ...
        try:
            res = demisto_client.generic_request_func(
                self=client, method="PUT", path="/settings/integration", body=module_instance
            )
        except ApiException:
...
def __delete_integration_instance(client, instance_id, logging_manager=logging):
    try:
        res = demisto_client.generic_request_func(
            self=client, method="DELETE", path="/settings/integration/" + urllib.parse.quote(instance_id)
        )
    except ApiException:
...
def __print_investigation_error(client, playbook_id, investigation_id, logging_manager):
    try:
        ...
        res = demisto_client.generic_request_func(
            self=client, method="POST", path="/investigation/" + urllib.parse.quote(investigation_id), body=empty_json
        )
        ...
        logging_manager.exception("Failed to print investigation error, error trying to communicate with demisto server")
...
def check_integration(
    ...
    incident, inc_id = __create_incident_with_playbook(client, f"inc_{playbook_id}", playbook_id, integrations, logging_module)
    ...
    while True:
        ...
        try:
            ...
            client = demisto_client.configure(
                base_url=client.api_client.configuration.host, api_key=client.api_client.configuration.api_key, verify_ssl=False
            )
...
def disable_all_integrations(dem_client, logging_manager=logging):
    ...
    try:
        body = {"size": 1000}
        int_resp = demisto_client.generic_request_func(
            self=dem_client, method="POST", path="/settings/integration/search", body=body
        )
        int_instances = ast.literal_eval(int_resp[0])
    ...
    for instance in int_instances["instances"]:
        if instance.get("enabled") == "true" and instance.get("isIntegrationScript"):
            logging_manager.debug(f'Adding to disable list. Name: {instance.get("name")}. Brand: {instance.get("brand")}')
