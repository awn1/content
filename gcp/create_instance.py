...
def options_handler():
    parser = argparse.ArgumentParser(description="A script that creates instances for the build")
    parser.add_argument(
        "--env-type",
        "-t",
    ...
    parser.add_argument("--outfile", help="path for te results file", required=True, default="./env_results.json")
    parser.add_argument("--filter-envs", help="path for the filter env file", required=True, default="./filter_envs.json")
    ...
    parser.add_argument("--instances-config", help="path to the instances config file", required=True)
    return parser.parse_args()


def instance_config(env_type: str, instances_config_file_path) -> List[Dict]:
...
def create_instances(inst_config: List[Dict], filtered_envs: Dict[str, bool], sa_file_path: str, zone: str) -> List[Dict]:
    pipeline_id = os.getenv("CI_PIPELINE_ID", "").lower() or f"local-dev-{uuid4()}"
