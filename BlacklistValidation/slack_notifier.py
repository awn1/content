...
def create_attachments_on_failure(secrets_filenames, matching_secrets, message, job_url, base_url):
    ...
    fields = (
        get_fields(secrets_filenames, matching_secrets, base_url)
    ...
    attachments = create_slack_attachments(
        color="danger",
        title="Master Secrets Test - Failure",
        fields=fields,
        job_url=job_url,
    )
    return attachments
...
def create_attachments_on_success(message, job_url):
    ...
    attachments = create_slack_attachments(
        color="good",
        title="Master Secrets Test - Success",
        fields=fields,
        job_url=job_url,
    )
    return attachments
...
def slack_notifier(
    slack_token,
    secrets_filenames=None,
    matching_secrets=None,
    message="",
    success=False,
    job_url=None,
):
    ...
    response = slack_client.chat_postMessage(channel=SLACK_CHANNEL, username=username, attachments=attachments)
