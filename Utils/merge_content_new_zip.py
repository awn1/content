...
def get_feature_branch_zip_file_path(feature_branch_build_path, zip_name):
    ...
    feature_zip_file_path = download_zip_file_from_gcp(current_feature_branch_zip_file_path, zip_destination_path, zip_name)
...
def option_handler():
    ...
    parser.add_argument("-b", "--build_number", help="The last successful build number of the feature branch", required=True)
...
def main():
    ...
    feature_branch_content_new_zip_file_path, content_new_zip_destination_path = get_feature_branch_zip_file_path(
        last_successful_feature_branch_build, "content_new"
    )

    feature_branch_all_content_zip_file_path, all_content_zip_destination_path = get_feature_branch_zip_file_path(
        last_successful_feature_branch_build, "all_content"
    )

    if feature_branch_content_new_zip_file_path:
        merge_zip_files(
            feature_branch_content_new_zip_file_path,
            artifacts_zip_path=CONTENT_NEW_ZIP_PATH,
    ...
    if feature_branch_all_content_zip_file_path:
        merge_zip_files(
            feature_branch_all_content_zip_file_path,
            artifacts_zip_path=ALL_CONTENT_ZIP_PATH,
