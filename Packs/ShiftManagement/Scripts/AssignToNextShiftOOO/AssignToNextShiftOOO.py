...
def main():
    ...
    if is_error(users_on_call):
        return_error(f"Failed to get users on call: {get_error(users_on_call)!s}")
    ...
    non_OOO_list = [
        x["username"] for x in users_on_call if x["username"] not in list_info and x["username"] not in away_user_names
    ]
