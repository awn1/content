...
def prepare_query(args: dict) -> str:
    ...
    for key, values in args.items():
        ...
        demisto.debug(f"prepare query Values: {values}, Type of value: {type(values)}")
        demisto.debug(f"prepare query Key: {key}, Type of key: {type(key)}")
        ...
        if key == "value":
            demisto.debug(len(values_as_list))
        ...
        demisto.debug(f"prepare query query: {query}")
        query_sections.append(query)

    query2 = " AND ".join(f"({qs})" for qs in query_sections) if query_sections else ""
    demisto.debug(f"prepare query query2: {query2}")
    return query2
...
def split_or_query(query: str, max_literals: int = 100) -> list[str]:
    ...
    Keeps an optional (type: ...) clause intact and splits only the OR part.

    Supported shapes:
      1) (value:"a" OR value:"b" OR ...)
      2) (type: x) AND (value:"a" OR value:"b" OR ...)
    ...
    # Try to capture a (type: ...) prefix if present
    m_type = re.match(r"^\(\s*type\s*:[^)]+\)\s*AND\s*\((.*)\)\s*$", q, flags=re.IGNORECASE | re.DOTALL)
    if m_type:
        type_match = re.search(r"^\(\s*type\s*:[^)]+\)", q, flags=re.IGNORECASE)
        if type_match:
            type_clause = type_match.group(0)
        else:
            demisto.debug(f"No type clause found in the query: {m_type}")
            type_clause = None

        value_part = m_type.group(1)
    else:
    ...
    for chunk in chunks:
        ...
        out.append(f"{type_clause} AND ({joined})" if type_clause else f"({joined})")
    return out


def search_indicators(args):
    # search for indicators
    demisto.debug(f"search_indicators Preparing query for args: {args}")
    ...
    demisto.debug(f"search_indicators indicators: {indicators}")
    ...
    demisto.debug(f"search_indicators filtered_indicators: {filtered_indicators}")
