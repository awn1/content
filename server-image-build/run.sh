...
time docker run -t --rm \
    --env CI=$CI --env XSOAR_ADMIN_PASS=$XSOAR_ADMIN_PASS --env BASE_INSTANCE_NAME=$BASE_INSTANCE_NAME --env INSTALLER_URL=$INSTALLER_URL --env SERVER_IMG_NAME=$SERVER_IMG_NAME \
