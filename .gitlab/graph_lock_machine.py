...
colors_dict = {
    "content-locks/locks-xsiam-ga": "red",
    "content-locks/locks-xsoar-ng": "blue",
    "content-locks/locks-xsoar-ng-nightly": "green",
    "content-locks/locks-xsiam-ga-nightly": "orange",
    "content-locks/locks-xsiam-ga-upload": "purple",
    "content-locks/locks-xsoar-ng-upload": "yellow",
}
labels_dict = {
    "content-locks/locks-xsiam-ga": "xsiam-ga",
    "content-locks/locks-xsoar-ng": "xsoar-ng",
...
def create_graphs(messages):
    ...
    for message in messages:
        if (
            "has joined the channel" in message
            or "Due to a high volume of activity, we are not displaying some messages sent by this application" in message
        ):
            continue
    ...
    # Create the graphs:
    timestamps2 = [
        datetime.strptime(item, "%Y-%m-%dT%H:%M:%S.%f").replace(tzinfo=pytz.utc).astimezone(ist) for item in timestamps2
    ]
    plt.legend(
        handles=[
    ...
    timestamps1 = [
        datetime.strptime(item, "%Y-%m-%dT%H:%M:%S.%f").replace(tzinfo=pytz.utc).astimezone(ist) for item in timestamps1
    ]
    plt.legend(
        handles=[
            plt.Line2D([0], [0], marker="o", color="w", markerfacecolor=color, markersize=6, label=labels_dict[type_])
    ...
    timestamps3 = [
        datetime.strptime(item, "%Y-%m-%dT%H:%M:%S.%f").replace(tzinfo=pytz.utc).astimezone(ist) for item in timestamps3
    ]
    plt.legend(
        handles=[
            plt.Line2D([0], [0], marker="o", color="w", markerfacecolor=color, markersize=6, label=labels_dict[type_])
...
def calculate_average(types, values):
    ...
    for type_, type_duration in type_values.items():
        if type_ in ["red", "blue"]:
            ...
            plt.axhline(
                y=average_value, color=type_, linestyle="--", label=f"Average {type_}"
